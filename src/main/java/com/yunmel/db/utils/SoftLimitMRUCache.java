...
public class SoftLimitMRUCache<K, V> implements Serializable {
  ...
  public SoftLimitMRUCache(int strongRefCount, int softRefCount) {
    ...
    if (strongRefCount > softRefCount) {
      throw new IllegalArgumentException(
          "Strong reference count cannot exceed soft reference count");
    }
  ...
  public synchronized V put(K key, V value) {
    if (key == null || value == null) {
      throw new NullPointerException(getClass().getName() + "does not support null key [" + key
          + "] or value [" + value + "]");
    }
    ...
    SoftReference<V> ref =
        softRefCache.put(key, new KeyedSoftReference(key, value, referenceQueue));
